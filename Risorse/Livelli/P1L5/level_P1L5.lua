...
-- FILENAME: level_P1L5.lua
-- LOCATION: .\.\Gioco\Risorse\Livelli\P1L5\level_P1L5.lua
-- DESCRIZIONE: pianeta 1 livello 5
--
-----------------------------------------------------------------------------------------
...
local function goToReload()
    ...
    composer.removeScene(".Risorse.Livelli.P1L5.reload")
    composer.gotoScene(".Risorse.Livelli.P1L5.reload")
    audio.stop()
...
local function winPopup()
    ...
    winScreen = display.newImageRect("/Risorse/Livelli/P1L5/Hai Vinto.png", 1920, 1080)
    winScreen.anchorX = 0
    winScreen.anchorY = 0
    winScreen.x = 0
    winScreen.y = 0
    ...
    continue = display.newImageRect("/Risorse/Livelli/P1L5/Continua.png", 388, 154)
    continue.anchorX = 0
    continue.anchorY = 0
    continue.x = 766
    continue.y = 671
...
local function wrongBall()
    -- caricamento, posizionamento e applicazione dissolvenza alla grafica per la vittoria
    loseScreen = display.newImageRect("/Risorse/Livelli/P1L5/Hai Perso.png", 1920, 1080)
    ...
    reset = display.newImageRect("/Risorse/Livelli/P1L5/Riprova.png", 388, 154)
    reset.anchorX = 0
    reset.anchorY = 0
    reset.x = 766
    reset.y = 671
...
-- Funzione per il errore pallina rossa
local function redCollision(event)
    local collidedObject = event.other
    if event.phase=="began" then
        if collidedObject.name=="red" then
            wrongBall()
        end
    end
end
...
function scene:create(event)
    ...
    mapData = json.decodeFile(system.pathForFile("Risorse/Livelli/P1L5/map_P1L5.json", system.ResourceDirectory))
...
function scene:show(event)
    ...
    elseif ( phase == "did" ) then -- se la scena è visualizzata...
        ...
        SFX_ballInHole = audio.loadSound("/Risorse/Livelli/P1L5/Palla buca.mp3")
        SFX_ballVSball = audio.loadSound("/Risorse/Livelli/P1L5/Palla contro palla.mp3")
        SFX_shore = audio.loadSound("/Risorse/Livelli/P1L5/Sponda.mp3")
        SFX_cue = audio.loadSound("/Risorse/Livelli/P1L5/Stecca.mp3")
        music = audio.loadSound("/Risorse/Livelli/P1L5/Musica.mp3")
        ...
        map = tiled.new(mapData, "Risorse/Livelli/P1L5") 
        ...
        whiteBall = display.newImageRect(sceneGroup, "/Risorse/Livelli/P1L5/Palla Bianca.png", 150, 150 )
        whiteBall.x = 659 
        whiteBall.y = 589
        whiteBall:toFront()
        ...
        whiteBall.collision = redCollision("white")
        whiteBall:addEventListener( "collision", redCollision)
        ...
        blackBall = display.newImageRect(sceneGroup, "/Risorse/Livelli/P1L5/Palla Nera.png", 150, 150 )
        blackBall.x = 1520 
        blackBall.y = 360
        blackBall:toFront()
        ...
        blackBall.collision = redCollision("black")
        blackBall:addEventListener( "collision", redCollision)

        -- caricamento della pallina rossa, posizionamento, messa in primo piano, assegnazione del corpo fisico, attivazione del collisore
        redBall = display.newImageRect(sceneGroup, "/Risorse/Livelli/P1L5/Palla Rossa.png", 150, 150 )
        redBall.x = 1129 
        redBall.y = 479
        redBall:toFront()
        redBall.name = "red"
        physics.addBody(redBall,"static", {radius=20})
        redBall.collision = ballHitBall("red")
        redBall:addEventListener( "collision", ballHitBall)

        -- caricamento della pallina rossa2, posizionamento, messa in primo piano, assegnazione del corpo fisico, attivazione del collisore
        redBall2 = display.newImageRect(sceneGroup, "/Risorse/Livelli/P1L5/Palla Rossa.png", 150, 150 )
        redBall2.x = 1129 
        redBall2.y = 700
        redBall2:toFront()
        redBall2.name = "red"
        physics.addBody(redBall,"static", {radius=20})
        redBall2.collision = ballHitBall("red")
        redBall2:addEventListener( "collision", ballHitBall)

        -- caricamento della stecca
        cue = display.newImageRect(sceneGroup, "/Risorse/Livelli/P1L5/SteccaP1.png", 26, 721)
        cue.anchorY = 0
        cue.x = 135
        cue.y = 240
        cue:toFront()
        ...
        resetButton = display.newImageRect(sceneGroup, "/Risorse/Livelli/P1L5/Freccia Bianca.png", 88, 88) 
        resetButton.x = 1724 
        resetButton.y = 46
        resetButton:toFront() 
        ...
        homeButton = display.newImageRect(sceneGroup, "/Risorse/Livelli/P1L5/Casa.png", 88, 88)
        homeButton.x = 1844 
        homeButton.y = 46
        homeButton:toFront() 
